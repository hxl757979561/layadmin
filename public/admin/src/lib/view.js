...
layui.define(["laytpl", "layer"], function(exports) {
    ...
    view.req = function(options) {
        ...
        if (request.tokenName) {
            ...
            var bearerToken = "Bearer " + layui.data(setter.tableName)[request.tokenName];
            options.headers[authKey] = authKey in options.headers ? options.headers[authKey] : bearerToken || "";
        }
            ...
            $.extend(
                {
                    ...
                    success: function(res) {
                        ...
                        //登录状态失效，清除本地 access_token，并强制跳转到登入页
                            ...
                            res[response.statusName] == statusCode.logout
                        ) {
                            layer.closeAll();
                            view.exit();
                        }
        ...
        for (var i = elemTemp.length; i > 0; i--) {
            (function() {
                var dataElem = elemTemp.eq(i - 1),
                    layDone =
                        dataElem.attr("lay-done") || dataElem.attr("lay-then"), //获取回调
                    url = laytpl(dataElem.attr("lay-url") || "").render(router), //接口 url
                    data = laytpl(dataElem.attr("lay-data") || "").render(
                        router
                    ), //接口参数
                    headers = laytpl(dataElem.attr("lay-headers") || "").render(
                        router
                    ); //接口请求的头信息
